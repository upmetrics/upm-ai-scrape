...
async function getVQD(query, ia = 'web', options) {
  try {
    const response = await needle_1.default('get', `https://duckduckgo.com/?${queryString({ q: query, ia })}`, {
      ...options,
      headers: {
        ...exports.COMMON_HEADERS,
        ...(options?.headers || {})
    ...
    const match = exports.VQD_REGEX.exec(body);

    if (!match) {
      console.error('❌ Failed to find VQD in response body.');
      console.error('🔍 Body Preview:', body.slice(0, 500)); // Print first 500 chars for inspection
      throw new Error('VQD token not found in response');
    }

    return match[1];
  } catch (e) {
    throw new Error(`Failed to get the VQD for query "${query}": ${e.message}`);
  }
}
